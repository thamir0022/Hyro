...
export default function Employee() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [employeeRes, ctcRes, attendanceRes, feedbacksRes] =
          await Promise.all([
        ...
        if (!employeeRes.ok)
          throw new Error(
            employeeData?.message || "Failed to fetch employee data"
          );
        if (!ctcRes.ok)
          throw new Error(ctcData?.message || "Failed to fetch CTC data");
        if (!attendanceRes.ok)
          throw new Error(
            attendanceData?.message || "Failed to fetch attendance data"
          );
        if (!feedbacksRes.ok)
          throw new Error(
            feedbacksData?.message || "Failed to fetch feedback data"
          );
